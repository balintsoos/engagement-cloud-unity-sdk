...
internal class MacosFileCache(private val fileManager: NSFileManager) : FileCacheApi {

    override fun get(fileName: String): ByteArray? {
        return getCacheDir()?.let { dir ->
            val path = "$dir/$fileName"
            if (exists(path)) readFromFile(path) else null
        }
    }

    override fun cache(fileName: String, file: ByteArray) {
        getCacheDir()?.let { dir ->
            writeToFile("$dir/$fileName", file)
        }
    }

    override fun remove(fileName: String) {
        getCacheDir()?.let { dir ->
            fileManager.removeItemAtPath("$dir/$fileName", null)
        }
    }
    ...
    private fun exists(path: String): Boolean = fileManager.fileExistsAtPath(path)
